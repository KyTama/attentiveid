...
docker compose --profile maintenance --env-file "$env_file" -f "$deploy_root/current/compose.production.yml" run --rm --no-deps seed-psychologists bun run dist/seed-psychologists.js --upsert
